...
// encoding, nothing DAW-specific — those belong to backend/daw, and this type
// only turns the result into text a button can display.
//
...
// means the command was written to the socket, not that REAPER received it —
// UDP has no delivery confirmation — so the success text points at REAPER
...
func (t *TransportService) report(command string, err error) string {
	...
	return fmt.Sprintf("Sent %s — check REAPER", command)
}
